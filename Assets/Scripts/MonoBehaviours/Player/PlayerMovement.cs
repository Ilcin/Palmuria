...
//    private readonly int hashLocomotionTag = Animator.StringToHash("Locomotion");
	...
	private readonly int hashWalkingPara = Animator.StringToHash("Walking");
    ...
    private void Start()
    {
		...
		if (playerSaveData.Load (startingPositionKey, ref startingPositionName)) {
...
//        agent.velocity = animator.deltaPosition / Time.deltaTime;
    ...
    private void Update()
    {
        ...
        // Otherwise, if the nav mesh agent is close to it's destination, call the Slowing function.
			...
			Slowing (agent.remainingDistance);
			isWalking = true;
		}
		...
		animator.SetBool (hashWalkingPara, isWalking);
		if (isWalking) {
			if (agent.isStopped) {
				spriteRenderer.flipX = (destinationPosition - transform.position).x < 0;
			} else {
...
//            transform.rotation = currentInteractable.interactionLocation.rotation;
			...
			currentInteractable.Interact(inventory.selectedItem ? inventory.selectedItem.item : null);
...
//        Quaternion targetRotation = Quaternion.LookRotation(agent.desiredVelocity);
    ...
    public void OnInteractableClick(Interactable interactable)
    {
		...
		interactionItem = (inventory.selectedItem) ? inventory.selectedItem.item : null;
