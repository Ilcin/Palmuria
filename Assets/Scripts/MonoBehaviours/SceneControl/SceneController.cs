...
public class SceneController : MonoBehaviour
{
    ...
    private IEnumerator Start ()
    {
		...
		if (SceneManager.sceneCount < 2) {
			yield return 
				(LoadSceneAndSetActive (startingSceneName, initialStartingPositionName));
		} else {
	...
	public void FadeAndLoadScene (string sceneName, string startingPositionName)
    {
        ...
        if (!isFading)
        {
            StartCoroutine (FadeAndSwitchScenes (sceneName, startingPositionName));
        }
    }


    // This is the coroutine where the 'building blocks' of the script are put together.
	private IEnumerator FadeAndSwitchScenes (string sceneName, string startingPositionName)
    {
		...
		yield return StartCoroutine (LoadSceneAndSetActive (sceneName, startingPositionName));
	...
	private IEnumerator LoadSceneAndSetActive (string sceneName, string startingPositionName)
    {
